...
```jsx
  22,345,412/s     1.00x   L.get(L_findHint_id_5000, ids)

   6,799,886/s     1.00x   R.reduceRight(add, 0, xs100)
     423,818/s    16.04x   L.foldr(add, 0, L.elems, xs100)
       4,014/s  1693.85x   O.Fold.foldrOf(O.Traversal.traversed, addC, 0, xs100)

      11,110/s     1.00x   R.reduceRight(add, 0, xs100000)
          53/s   210.02x   L.foldr(add, 0, L.elems, xs100000)
           0/s Infinityx   O.Fold.foldrOf(O.Traversal.traversed, addC, 0, xs100000) -- STACK OVERFLOW

     708,479/s     1.00x   L.foldl(add, 0, L.elems, xs100)
     201,754/s     3.51x   R.reduce(add, 0, xs100)
       2,759/s   256.76x   O.Fold.foldlOf(O.Traversal.traversed, addC, 0, xs100)

   3,460,794/s     1.00x   L.sum(L.elems, xs100)
   2,674,840/s     1.29x   K.traversed().sumOf(xs100)
     487,092/s     7.11x   L.concat(Sum, L.elems, xs100)
     182,746/s    18.94x   xs100.reduce((a, b) => a + b, 0)
     103,931/s    33.30x   R.sum(xs100)
      23,252/s   148.84x   P.sumOf(P.traversed, xs100)
       4,449/s   777.86x   O.Fold.sumOf(O.Traversal.traversed, xs100)

     553,300/s     1.00x   L.maximum(L.elems, xs100)
       3,354/s   164.99x   O.Fold.maximumOf(O.Traversal.traversed, xs100)

     145,414/s     1.00x   L.sum([L.elems, L.elems, L.elems], xsss100)
     143,626/s     1.01x   L.concat(Sum, [L.elems, L.elems, L.elems], xsss100)
       4,659/s    31.21x   P.sumOf(R.compose(P.traversed, P.traversed, P.traversed), xsss100)
         875/s   166.15x   O.Fold.sumOf(R.compose(O.Traversal.traversed, O.Traversal.traversed, O.Traversal.traversed), xsss100)

   3,046,375/s     1.00x   K.traversed().arrayOf(xs100)
     264,627/s    11.51x   L.collect(L.elems, xs100)
      28,314/s   107.59x   xs100.map(I.id)
       3,521/s   865.22x   O.Fold.toListOf(O.Traversal.traversed, xs100)

     107,679/s     1.00x   L.collect([L.elems, L.elems, L.elems], xsss100)
      25,711/s     4.19x   (() => { let acc = []; xsss100.forEach(x0 => { x0.forEach(x1 => { acc = acc.concat(x1); })}); return acc; })()
      25,274/s     4.26x   K.traversed().traversed().traversed().arrayOf(xsss100)
       9,540/s    11.29x   R.chain(R.chain(R.identity), xsss100)
         793/s   135.73x   O.Fold.toListOf(R.compose(O.Traversal.traversed, O.Traversal.traversed, O.Traversal.traversed), xsss100)

      64,339/s     1.00x   R.flatten(xsss100)
      32,063/s     2.01x   L.collect(flatten, xsss100)

  15,234,325/s     1.00x   L.modify(L.elems, inc, xs)
   1,720,746/s     8.85x   R.map(inc, xs)
     810,811/s    18.79x   xs.map(inc)
     456,199/s    33.39x   K.traversed().over(xs, inc)
     442,028/s    34.46x   P.over(P.traversed, inc, xs)
     391,029/s    38.96x   O.Setter.over(O.Traversal.traversed, inc, xs)

     416,268/s     1.00x   L.modify(L.elems, inc, xs1000)
     124,385/s     3.35x   R.map(inc, xs1000)
       2,859/s   145.60x   xs1000.map(inc)
       2,851/s   145.99x   K.traversed().over(xs1000, inc)
         396/s  1052.43x   O.Setter.over(O.Traversal.traversed, inc, xs1000) -- QUADRATIC
         368/s  1131.70x   P.over(P.traversed, inc, xs1000) -- QUADRATIC

     150,383/s     1.00x   L.modify([L.elems, L.elems, L.elems], inc, xsss100)
       9,769/s    15.39x   R.map(R.map(R.map(inc)), xsss100)
       7,796/s    19.29x   K.traversed().traversed().traversed().over(xsss100, inc)
       7,692/s    19.55x   xsss100.map(x0 => x0.map(x1 => x1.map(inc)))
       3,522/s    42.70x   P.over(R.compose(P.traversed, P.traversed, P.traversed), inc, xsss100)
       2,939/s    51.16x   O.Setter.over(R.compose(O.Traversal.traversed, O.Traversal.traversed, O.Traversal.traversed), inc, xsss100)

  35,034,296/s     1.00x   L.get(1, xs)
  26,418,109/s     1.33x   _get(xs, 1)
   3,832,381/s     9.14x   R.nth(1, xs)
   1,484,780/s    23.60x   R.view(l_1, xs)
     774,330/s    45.24x   K.idx(1).get(xs)

  54,846,939/s     1.00x   L_get_1(xs)
  20,456,991/s     2.68x   L.get(1)(xs)
   3,914,806/s    14.01x   R_nth_1(xs)
   2,725,900/s    20.12x   R.nth(1)(xs)

  23,090,453/s     1.00x   L.set(1, 0, xs)
   6,750,531/s     3.42x   R.update(1, 0, xs)
   5,961,043/s     3.87x   (() => { let ys = xs.slice(); ys[1] = 0; return ys; })()
     969,360/s    23.82x   R.set(l_1, 0, xs)
     822,150/s    28.09x   xs.map((x, i) => i === 1 ? 0 : x)
     564,380/s    40.91x   K.idx(1).set(xs, 0)

  38,484,171/s     1.00x   L.get("y", xyz)
  24,981,220/s     1.54x   R.prop("y", xyz)
   9,787,210/s     3.93x   _get(xyz, "y")
   2,423,574/s    15.88x   R.view(l_y, xyz)
     719,303/s    53.50x   K.key("y").get(xyz)

  67,092,805/s     1.00x   L_get_y(xyz)
  24,328,419/s     2.76x   R_prop_y(xyz)
  19,130,893/s     3.51x   L.get("y")(xyz)
   9,333,533/s     7.19x   R.prop("y")(xyz)

  11,275,688/s     1.00x   R.assoc("y", 0, xyz)
  11,057,609/s     1.02x   L.set("y", 0, xyz)
   1,385,145/s     8.14x   R.set(l_y, 0, xyz)
     575,901/s    19.58x   K.key("y").set(xyz, 0)

  14,495,204/s     1.00x   R.path([0,"x",0,"y"], axay)
  14,433,359/s     1.00x   L.get([0,"x",0,"y"], axay)
  12,480,615/s     1.16x   _get(axay, [0,"x",0,"y"])
   2,252,257/s     6.44x   R.view(l_0x0y, axay)
     472,015/s    30.71x   R.view(l_0_x_0_y, axay)
     199,395/s    72.70x   K.idx(0).key("x").idx(0).key("y").get(axay)

   3,946,840/s     1.00x   L.set([0,"x",0,"y"], 0, axay)
     943,969/s     4.18x   R.assocPath([0,"x",0,"y"], 0, axay)
     581,138/s     6.79x   R.set(l_0x0y, 0, axay)
     333,360/s    11.84x   R.set(l_0_x_0_y, 0, axay)
     171,496/s    23.01x   K.idx(0).key("x").idx(0).key("y").set(axay, 0)

   3,918,224/s     1.00x   L.modify([0,"x",0,"y"], inc, axay)
     607,591/s     6.45x   R.over(l_0x0y, inc, axay)
     345,212/s    11.35x   R.over(l_0_x_0_y, inc, axay)
     171,221/s    22.88x   K.idx(0).key("x").idx(0).key("y").over(axay, inc)

  23,746,631/s     1.00x   L.remove(1, xs)
   3,188,514/s     7.45x   R.remove(1, 1, xs)

  11,210,427/s     1.00x   L.remove("y", xyz)
   2,532,395/s     4.43x   R.dissoc("y", xyz)

  16,619,830/s     1.00x   L.get(["x","y","z"], xyzn)
  15,184,229/s     1.09x   _get(xyzn, ["x", "y", "z"])
  14,061,375/s     1.18x   R.path(["x","y","z"], xyzn)
   2,248,408/s     7.39x   R.view(l_xyz, xyzn)
     821,649/s    20.23x   R.view(l_x_y_z, xyzn)
     252,885/s    65.72x   K.key("x").key("y").key("z").get(xyzn)
     163,705/s   101.52x   O.Getter.view(o_x_y_z, xyzn)

   4,655,439/s     1.00x   L.set(["x","y","z"], 0, xyzn)
   1,786,472/s     2.61x   R.assocPath(["x","y","z"], 0, xyzn)
     815,381/s     5.71x   R.set(l_xyz, 0, xyzn)
     575,822/s     8.08x   R.set(l_x_y_z, 0, xyzn)
     216,684/s    21.48x   K.key("x").key("y").key("z").set(xyzn, 0)
     215,385/s    21.61x   O.Setter.set(o_x_y_z, 0, xyzn)

   1,077,436/s     1.00x   R.find(x => x > 3, xs100)
     195,553/s     5.51x   L.selectAs(x => x > 3 ? x : undefined, L.elems, xs100)
       2,767/s   389.36x   O.Fold.findOf(O.Traversal.traversed, x => x > 3, xs100)

   3,835,124/s     1.00x   L.selectAs(x => x < 3 ? x : undefined, L.elems, xs100)
   3,673,357/s     1.04x   R.find(x => x < 3, xs100)
       2,716/s  1412.21x   O.Fold.findOf(O.Traversal.traversed, x => x < 3, xs100) -- NO SHORTCUT EVALUATION

   4,019,997/s     1.00x   L.remove(50, xs100)
   1,877,085/s     2.14x   R.remove(50, 1, xs100)

   4,310,264/s     1.00x   L.set(50, 2, xs100)
   1,657,842/s     2.60x   R.update(50, 2, xs100)
     658,215/s     6.55x   R.set(l_50, 2, xs100)
     474,341/s     9.09x   K.idx(50).set(xs100, 2)
```
